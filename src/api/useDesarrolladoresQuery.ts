import { useQuery } from '@tanstack/react-query';
import axios from 'axios';
import { useDesarrolladoresStore } from '../store/useDesarrolladoresStore';

const API_URL = 'https://apipruebas.rbu.cl/api/desarrolladores';
const TOKEN = 'T7fZ9gHj5KmN2pQr8sV3uW6xY1zA4bC0dE7fG9hJ2kL4mN6pQ8rS0tV3wX5yZ7aC9';

export const useDesarrolladoresQuery = () => {
  const setDesarrolladores = useDesarrolladoresStore((state) => state.setDesarrolladores);

  return useQuery({
    queryKey: ['desarrolladores'],
    queryFn: async () => {
      const response = await axios.get(API_URL, {
        headers: {
          Authorization: `Bearer ${TOKEN}`,
        },
      });
      setDesarrolladores(response.data);
      return response.data;
    },
  });
};
